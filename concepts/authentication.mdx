...
The server middleware (`verificarToken`) extracts the header, verifies the JWT signature and expiry, and attaches the decoded user payload to the request before passing it to the route handler. Requests that omit the header or present a malformed value are rejected immediately with `401`.
...
<ResponseField name="403 Forbidden" type="object">
  Returned when the request is authenticated (valid token) but the user's role does not meet the requirement of the endpoint being accessed.
