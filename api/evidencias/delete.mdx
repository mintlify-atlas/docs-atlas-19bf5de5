---
title: "DELETE /api/expedientes/:no_siniestro/evidencias — Remove Evidence Files"
sidebarTitle: "DELETE /evidencias"
description: "Delete a single evidence record by ID, or remove the entire evidence folder for an expediente. Single deletion requires Administrador; folder deletion requires Administrador or Operador."
---
...
Deletes the evidence record from the database **and** attempts to remove the physical file from disk. If the stored `ruta` path exists on the filesystem, the file is deleted with `fs.unlinkSync` before the database record is removed.
...
<ParamField path="no_siniestro" type="string" required>
  The siniestro number of the expediente whose evidence folder should be removed (e.g. `SIN-2024-001`). The server validates that the resolved path stays within the `EVIDENCIAS_BASE` directory to prevent path traversal attacks.
</ParamField>
