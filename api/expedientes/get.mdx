---
title: "GET /api/expedientes/:no_siniestro — Retrieve a Single Expediente"
sidebarTitle: "GET /expedientes/:id"
description: "Fetch a single expediente by its siniestro number, including related checklists, evidence metadata, and levantamiento records. Requires authentication."
---

Retrieve a single expediente and all of its related records in one request. Unlike the list endpoint, this response includes the full set of scalar fields plus nested relations: `aseguradora`, `historial_estado` (audit history, newest-first), and `evidencia` (file metadata). All authenticated roles may call this endpoint.
