...
description: "Create a new Monitor API user with a specified role. Requires an active Administrador JWT token. Returns the created user profile without the password hash."
---
...
`POST /api/auth/register` — **requires `Authorization: Bearer <token>` with `Administrador` role.**
